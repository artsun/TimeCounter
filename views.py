...
class Wday(db.Model):
    ...
    def calc_finish(self):
        ...
        # TODO:  предусмотреть год
        if (h // 24) and monthrange(self.start.year, self.start.month)[1] == self.start.day:
            month, days = self.start.month + 1, 1
        else:
            days = (h // 24) + self.start.day
            month = self.start.month
        h = h % 24 if h // 24 else h
        return self.start.replace(hour=h, day=days, month=month) + self.calc_breaks()
